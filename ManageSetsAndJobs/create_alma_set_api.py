...
def main():
    ...
    for line in configuration:
        ...
        if m:
            ...
            if m.group(1) == "apikey":
                apikey = m.group(2)
            if m.group(1) == "set_name":
                name = m.group(2)
    set_xml = str('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><set link="/almaws/v1/conf/sets/6491540120001021"><name>' + name + '</name><type desc="Itemized">ITEMIZED</type><content desc="Physical items">ITEM</content><private desc="Yes">true</private><status desc="Active">ACTIVE</status></set>')
    try:
